...
async function main() {
  ...
  const dataEntries = (results.data as string[][]).map((row: string[]) => ({
    cityName: row[0],
    country: row[1],
    ...
    volkswagenGolf14_90KWTrendlineOrEquivalentNewCarUSD:
      parseFloat(row[35]) || null,
    toyotaCorollaSedan16l97kWComfortOrEquivalentNewCarUSD:
      parseFloat(row[36]) || null,
    basicElectricityHeatingCoolingWaterGarbageFor85m2ApartmentUSD:
      parseFloat(row[37]) || null,
    prepaidMobileTariffLocalNoDiscountsOrPlans1MinUSD:
    ...
    preschoolOrKindergartenFullDayPrivateMonthlyFor1ChildUSD:
      parseFloat(row[43]) || null,
    ...
    pricePerSquareMeterToBuyApartmentInCityCentreUSD:
      parseFloat(row[53]) || null,
    pricePerSquareMeterToBuyApartmentOutsideOfCentreUSD:
      parseFloat(row[54]) || null,
    averageMonthlyNetSalaryAfterTaxUSD: parseFloat(row[55]) || null,
    mortgageInterestRateInPercentagesYearlyFor20YearsFixedRate:
      parseFloat(row[56]) || null,
    dataQuality: parseInt(row[57]) || null,
    latitude: parseFloat(row[58]) || null,
    longitude: parseFloat(row[59]) || null,
    population: parseFloat(row[60]) || null,
  }));
